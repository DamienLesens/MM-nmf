...
#algs = ["mSOM", "MUSOM", "PGD", "NeNMF", "MU", "HALS"]
...
def one_run(**cfg):
    ...
    Wgt = Wgt[:, 27:(27+cfg["rank"])]  # octaves in the middle starting from C2, 2 octaves from lowest piano key 
    ...
    lamb = opt_scaling_fro(Y, Wgt@Hini)
    Hini = lamb*Hini
    ...
    H0, _, _, _, error0, toc0 = nn_fac.nnls.hals_nnls_acc(WtV, WtW, np.copy(Hini), maxiter=cfg["NbIter"], return_error=True, delta=cfg["delta"], M=Y)
    toc0 = [toc0[i] + toc0_offset for i in range(len(toc0))] # leave the 0 in place for init
    toc0[0]=0

    # MU
    error1, H1, toc1 = nls_f.NMF_Lee_Seung(Y,  Wgt, Hini, cfg["NbIter"], legacy=False, delta=cfg["delta"], verbose=verbose, epsilon=cfg["epsilon"])
    
    # MUSOM
    error2, H2, toc2 = nls_f.NMF_proposed_Frobenius(Y, Wgt, Hini, cfg["NbIter"], delta=cfg["delta"], verbose=verbose, method="MUSOM", gamma=1.9, epsilon=cfg["epsilon"])
   
    # NeNMF 
    error3, H3, toc3  = nls_f.NeNMF(Y, Wgt, Hini, itermax=cfg["NbIter"], delta=cfg["delta"], verbose=verbose, epsilon=cfg["epsilon"])
    
    # PGD 
    error4, H4, toc4  = nls_f.Grad_descent(Y , Wgt, Hini, cfg["NbIter"], delta=cfg["delta"], verbose=verbose, epsilon=cfg["epsilon"])
    
    # mSOM
    error5, H5, toc5 = nls_f.NMF_proposed_Frobenius(Y, Wgt, Hini, cfg["NbIter"], delta=cfg["delta"], verbose=verbose, method="mSOM", gamma=1.9, epsilon=cfg["epsilon"])
    ...
    #error0, H0, toc0 = nls_f.NMF_proposed_Frobenius(Y, Wgt, np.copy(Hini), cfg["NbIter"], delta=cfg["delta"], verbose=verbose, gamma=1.9, epsilon=cfg["epsilon"])
    #error2, H2, toc2 = nls_f.NMF_proposed_Frobenius(Y, Wgt, np.copy(Hini), cfg["NbIter"], method="MUSOM", delta=cfg["delta"], verbose=verbose, gamma=1.9, epsilon=cfg["epsilon"])
    #error3, H3, toc3 = nls_f.Grad_descent(Y, Wgt, np.copy(Hini), cfg["NbIter"],  epsilon=cfg["epsilon"], verbose=verbose, delta=cfg["delta"], gamma=1.9)
    #error4, H4, toc4 = nls_f.NeNMF(Y, Wgt, np.copy(Hini), itermax=cfg["NbIter"], epsilon=cfg["epsilon"], verbose=verbose, delta=cfg["delta"])
    #error5, H5, toc5 = nls_f.NMF_Lee_Seung(Y,  Wgt, np.copy(Hini), cfg["NbIter"], legacy=False, delta=cfg["delta"], verbose=verbose, epsilon=cfg["epsilon"])
    ...
    #H6, _, _, _, error6, toc6 = nn_fac.nnls.hals_nnls_acc(WtV, WtW, np.copy(Hini), maxiter=cfg["NbIter"], return_error=True, delta=cfg["delta"], M=Y)
...
scale, scale_text = 1.6, 2.1  # size of the plots
...
fig = pp.performance_profiles(df, variables=variables_plot, n_cols=n_cols, threshold_points=threshold_points, algorithms=algorithms, pad_x_title=0.02)

fig.update_layout(
    title="Audio Frobenius NLS, winner profiles",
    xaxis_type="log",
...
ovars=["rank"]
df_kl_conv = pp.df_to_convergence_df(df, groups=True, groups_names=ovars, other_names=ovars,
...
pxfig2.layout.title.text = "Audio Frobenius NLS, convergence plots"
pxfig2.layout.title.font.size = 10*scale_text
pxfig2.update_layout(margin_t=100)
#pxfig2.update_xaxes(exponentformat='power')
#pxfig2.update_yaxes(exponentformat='power')

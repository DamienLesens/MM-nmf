...
name = "hsi_nls_frobenius_19_01_2026"
...
def one_run(**cfg):
    ...
    H0, _, _, _, error0, toc0 = nn_fac.nnls.hals_nnls_acc(WtV, WtW, np.copy(Hini), maxiter=cfg["NbIter"], return_error=True, delta=cfg["delta"], M=M)
    toc0 = [toc0[i] + toc0_offset for i in range(len(toc0))] # leave the 0 in place for init
    toc0[0]=0

    # MU
    error1, H1, toc1 = nls_f.NMF_Lee_Seung(M,  Wref, Hini, cfg["NbIter"], legacy=False, delta=cfg["delta"], verbose=verbose, epsilon=cfg["epsilon"])
    
    # MUSOM
    error2, H2, toc2 = nls_f.NMF_proposed_Frobenius(M, Wref, Hini, cfg["NbIter"], delta=cfg["delta"], verbose=verbose, method="MUSOM", gamma=1.9, epsilon=cfg["epsilon"])
   
    # NeNMF 
    error3, H3, toc3  = nls_f.NeNMF(M, Wref, Hini, itermax=cfg["NbIter"], delta=cfg["delta"], verbose=verbose, epsilon=cfg["epsilon"])
    
    # PGD 
    error4, H4, toc4  = nls_f.Grad_descent(M , Wref, Hini, cfg["NbIter"], delta=cfg["delta"], verbose=verbose, epsilon=cfg["epsilon"])
    
    # mSOM
    error5, H5, toc5 = nls_f.NMF_proposed_Frobenius(M, Wref, Hini, cfg["NbIter"], delta=cfg["delta"], verbose=verbose, method="mSOM", gamma=1.9, epsilon=cfg["epsilon"])
...
fig.update_layout(
    title="HSI Frobenius NLS, winner profiles",
    xaxis_type="log",
...
pxfig.layout.title.text = "HSI Frobenius NLS, convergence plots"
#pxfig.layout.title.text = "Convergence plots"
